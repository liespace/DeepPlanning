...
class DataPlotter(object):
    ...
    def plot_obj_loss_val(self):
        ...
        files.sort()
        colors = ['black', 'purple', 'green', 'blue', 'red', 'grey']
        ...
        for i, f in enumerate(files):
            ...
            labels.append('$\\lambda_r$ ={:1.0e}'.format(lamb))
        ax.set_ylim([0.1, 0.7])
        ax.set_yticks(np.arange(0.1, 0.7, 0.1))
        plt.legend(handles, labels, prop={'size': self.fontsize}, ncol=2)
    ...
    def plot_cor_loss(self):
        ...
        for i, f in enumerate(files):
            ...
            labels.append('$\\lambda_r$ ={:1.0e}'.format(lamb))
        # ax.set_ylim([0.0, 0.])
        ax.set_yticks(np.arange(0.22, 0.24, 0.005))
        plt.legend(handles, labels, prop={'size': self.fontsize}, ncol=2)
