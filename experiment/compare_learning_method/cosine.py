...
class DataPlotter(object):
    ...
    def plot_obj_loss_val(self):
        ...
        files.sort()
        colors = ['blue', 'grey', 'black', 'red', 'green']
        ax, handles = self.new_figure(y_label='$J_c$[-]'), []
        labels = ['cosine:200-fr:75',
                  'cosine:250-fr:75', 'cosine:200-fr:100', 'cosine:150-fr:75', 'dc:10-ep:[75_105_135]-fr:75']
        ends = [300, 300, 300, 300, 300]
        ...
        ax.set_ylim([0.148, 0.220])
        ax.set_xlim([0, 270])
        ax.set_yticks(np.arange(0.15, 0.219, 0.02))
        ax.set_xticks(np.arange(100, 300, 100))
        plt.xticks(list(plt.xticks()[0]) + [75, 100, 150, 250])
        ax.vlines(100, 0, 0.17, colors='black', linestyles='dashed', lw=10)
        ax.vlines(75, 0, 0.17, colors='black', linestyles='dashed', lw=10)
    ...
    def plot_cor_loss_val(self):
        ...
        for i, f in enumerate(files):
            ...
            data2d[:, 1] /= float(lamb)
            handles.append(self.plotting(ax=ax, data2d=data2d, end=ends[i], color=colors[i]))
        ax.set_ylim([0.2198, 0.223])
        ax.set_xlim([15, 260])
        ax.set_yticks(np.arange(0.220, 0.2229, 0.0005))
        ax.set_xticks(np.arange(100, 300, 100))
        plt.xticks(list(plt.xticks()[0]) + [75, 100, 150, 250])
        ax.vlines(100, 0, 0.2207, colors='black', linestyles='dashed', lw=10)
        ax.vlines(75, 0, 0.2207, colors='black', linestyles='dashed', lw=10)
        plt.legend(handles, labels, prop={'size': self.fontsize}, ncol=1)
