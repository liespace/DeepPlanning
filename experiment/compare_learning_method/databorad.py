...
class DataPlotter(object):
    ...
    def plot_obj_loss_val(self):
        ...
        labels = ['dc:10-ep:[75_105_135]', 'dc:10-ep:[100_110_140]-fr:100',
                  'dc:10-ep:[50_100_160]-fr:50', 'dc:10-ep:[75_105_135]-fr:75']
        ends = [300, 220, 300, 200]
        ...
        ax.set_ylim([0.148, 0.172])
        ax.set_xlim([30, 320])
        ax.set_yticks(np.arange(0.15, 0.17, 0.01))
        ax.set_xticks(np.arange(100, 300, 100))
        plt.xticks(list(plt.xticks()[0]) + [50, 75, 100])
        ax.vlines(100, 0, 0.156, colors='grey', linestyles='dashed', lw=10)
        ax.vlines(50, 0, 0.166, colors='green', linestyles='dashed', lw=10)
        ax.vlines(75, 0, 0.159, colors='red', linestyles='dashed', lw=10)
    ...
    def plot_cor_loss_val(self):
        ...
        for i, f in enumerate(files):
            ...
            data2d[:, 1] /= float(lamb)
            handles.append(self.plotting(ax=ax, data2d=data2d, end=ends[i], color=colors[i]))
        ax.set_ylim([0.2198, 0.222])
        ax.set_xlim([30, 310])
        ax.set_yticks(np.arange(0.220, 0.2218, 0.0005))
        ax.set_xticks(np.arange(100, 300, 100))
        plt.xticks(list(plt.xticks()[0]) + [50, 75, 100])
        ax.vlines(100, 0, 0.2205, colors='grey', linestyles='dashed', lw=10)
        ax.vlines(50, 0, 0.2205, colors='green', linestyles='dashed', lw=10)
        ax.vlines(75, 0, 0.22056, colors='red', linestyles='dashed', lw=10)
        plt.legend(handles, labels, prop={'size': self.fontsize}, ncol=1)
