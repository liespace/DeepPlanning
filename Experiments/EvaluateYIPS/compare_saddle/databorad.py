...
class DataPlotter(object):
    ...
    def plot_obj_loss_val(self):
        ...
        ax, handles, labels = self.new_figure(y_label='$J_c$[-]'), [], ['dc:10-ep:50', 'dc:null', 'dc:10-ep:50-fr:50']
        ...
        ax.set_ylim([0.150, 0.235])
        ax.set_yticks(np.arange(0.150, 0.235, 0.02))
        ax.vlines(50, 0, 1e2, colors='k', linestyles='dashed', lw=8)
    ...
    def plot_cor_loss_val(self):
        ...
        files.sort()
        colors = ['red', 'green', 'blue']
        ax, handles, labels = self.new_figure(), [], ['dc:10-ep:50', 'dc:null', 'dc:10-ep:50-fr:50']
        for i, f in enumerate(files):
            data2d, lamb = self.read_csv(f), float(f.split('bce_')[-1].split('_')[0])
            data2d[:, 1] /= float(lamb)
            handles.append(self.plotting(ax=ax, data2d=data2d, end=200, color=colors[i]))
            # lr = float(f.split('adam_')[-1].split(')')[0])
            # labels.append('$lr$ ={:1.0e}'.format(lr))
        ax.set_ylim([0.2195, 0.223])
        ax.set_yticks(np.arange(0.2195, 0.223, 0.001))
        ax.vlines(50, 0, 1e2, colors='k', linestyles='dashed', lw=8)
        plt.legend(handles, labels, prop={'size': self.fontsize}, ncol=1)
    ...
    def plot_cor_loss(self):
        ...
        for i, f in enumerate(files):
            ...
            # lr = float(f.split('adam_')[-1].split(')')[0])
            # labels.append('$lr$ ={:1.0e}'.format(lr))
        ax.set_ylim([0.217, 0.220])
        ax.set_yticks(np.arange(0.217, 0.219, 0.001))
        ax.vlines(50, 0, 1e2, colors='k', linestyles='dashed', lw=8)
        plt.legend(handles, labels, prop={'size': self.fontsize}, ncol=1)
