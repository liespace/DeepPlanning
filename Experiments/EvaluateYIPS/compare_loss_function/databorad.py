...
class DataPlotter(object):
    ...
    def plot_obj_loss_val(self):
        ...
        labels = ['BCE', 'MSE', 'MSE-Step', 'BCE-Step']
        ends = [200, 250, 300, 200]
        for i, f in enumerate(files):
            data2d, lamb = self.read_csv(f), float(f.split('e_')[-1].split('_')[0])
            handles.append(self.plotting(ax=ax, data2d=data2d, end=ends[i], color=colors[i]))
        ax.set_ylim([0.145, 0.225])
        ax.set_yticks(np.arange(0.15, 0.225, 0.05))
    ...
    def plot_obj_loss(self):
        ...
        ax.set_ylim([0., 0.3])
        ax.set_xlim([0, 310])
        ax.set_yticks(np.arange(0., 0.29, 0.1))
        plt.legend(handles, labels, prop={'size': self.fontsize}, ncol=1)
    ...
    def plot_cor_loss_val(self):
        ...
        files.sort()
        colors = ['green', 'black', 'red', 'blue']
        ax, handles = self.new_figure('$g(\,J_r)$[-]'), []
        labels = ['BCE', 'MSE', 'MSE-Step', 'BCE-Step']
        ends = [200, 250, 300, 200]
        for i, f in enumerate(files):
            print(f)
            data2d, lamb = self.read_csv(f), float(f.split('e_')[-1].split('_')[0])
            data2d[:, 1] = (data2d[:, 1] - min(data2d[:, 1])) / (max(data2d[:, 1]) - min(data2d[:, 1]))
            handles.append(self.plotting(ax=ax, data2d=data2d, end=ends[i], color=colors[i]))
        ax.set_ylim([-0.02, 0.5])
        ax.set_xlim([0, 310])
        ax.set_yticks(np.arange(0., 0.49, 0.1))
        plt.legend(handles, labels, prop={'size': self.fontsize}, ncol=1)
