...
def main(dataset_folder, inputs_filename, outputs_tag, rounds):
    ...
    for i, seq in enumerate(seqs):
        ...
        np.savetxt('{}/{}_summary.txt'.format(outputs_folder, seq), [result, runtime, [np.multiply(result, runtime).mean()]*rounds], delimiter=',')
        couples.append((np.mean(runtime) if np.mean(result) >= 1 else np.inf, int(seq)))
    print(sorted(couples))
    np.savetxt('{}/summary.txt'.format(outputs_folder), couples, delimiter=',')
