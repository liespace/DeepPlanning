...
def main(dataset_folder='dataset', outputs_tag='ose'):
    output_folder = 'heuristics' + os.sep + outputs_tag
    explorer = OSExplorer(
    ...
    for i, seq in enumerate(seqs):
        ...
        past = time.time()
        result = explorer.exploring()
        runtime = time.time() - past
        failed_seqs.append(seq) if not result else None
    ...
    np.savetxt('{}/failed_seqs.txt'.format(output_folder), failed_seqs, delimiter=',')
