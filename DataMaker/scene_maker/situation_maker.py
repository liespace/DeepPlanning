        ...
        with CarlaRunner(self.world, self.sensors, fps=10, fs=10) as sync_mode:
            for source in self._source_builder():
                ...
                for extras in self._extras_builder():
                    ...
                    snapshot, depth_f, depth_r, depth_b, depth_l = sync_mode.tick(timeout=2.0)
                    # retrieve the data of sensors
                    archiver = OTArchiver()
                    seq = self.get_seq()
                    archiver.archive_task(source, self.target, seq)
                    rotation = (self.player.get_transform().rotation.roll,
                                -self.player.get_transform().rotation.pitch, 0.)
                    try:
                        hor = self.scene['hor']
                    except KeyError:
                        hor = None
                    ogm = archiver.archive_ogm(depthmaps=[depth_f, depth_r, depth_b, depth_l],
                                               infos=self.script['sensors'][:],
                                               seq=seq, rotation=rotation, hor=hor)
...
def main():
    ...
    director = SceneDirector('./scene_maker/script.json', world=world, client=client)
