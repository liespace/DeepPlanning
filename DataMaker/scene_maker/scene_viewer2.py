...
def draw_a_arrow(world, transform, color):
    ...
    # world.debug.draw_arrow(begin=transform.location + carla.Location(z=0.2),
    #                        end=transform.location + carla.Location(x=math.cos(yaw), y=math.sin(yaw), z=0.2),
    #                        thickness=0.05, arrow_size=0.2,
    #                        color=color)
...
def draw_prediction(world, source, no=0):
    filepath = '{}/{}_pred.txt'.format('scene_maker', no)
    ...
    for p in pred:
        if p[3] > confidence:
            ...
            draw_box_with_arrow2(world, state2transform(state, z=0.3), carla.Color(r=255, b=0, g=0, a=255), text='pred')


def draw_trajectory(world, no=0):
    filepath = '{}/{}_trajectory.txt'.format('scene_maker', no)
    if not os.path.isfile(filepath):
        print('no trajectory: {}'.format(filepath))
        return
    tj = numpy.loadtxt(filepath)
...
def main():
    ...
    while True:
        ...
        try:
            ...
            draw_box_with_arrow(world, target_point, carla.Color(r=0, b=255, g=0, a=255), 'Target')
